---
nombreComun: "Neem / Nim"
nombreCientifico: "Azadirachta indica"
nombresAlternativos: ["Nim"]
taxonomia:
  reino: "Plantae"
  division: "Magnoliophyta"
  clase: "Magnoliopsida"
  familia: "Meliaceae"
  genero: "Azadirachta"
etnobotanica:
  clasificacion: "Medicinal y agroecológica"
  parteUtilizada: "Hojas, corteza, semillas y aceite"
  usoTradicional: "Antimicrobiana, antifúngica, antiparasitaria y antiinflamatoria"
perfilEtnobotanico:
  |
    Las hojas y corteza se aplican tópicamente para afecciones dermatológicas severas. En agroecología, los extractos de sus semillas actúan como reguladores del crecimiento de insectos plaga.
historiaEvolucion:
  origen:
    |
      Originario de las regiones áridas de la India, Myanmar y Pakistán, con registros milenarios en la medicina Ayurvédica.
  dispersion:
    |
      Introducido en el siglo XX en regiones áridas de África y América Latina para mitigar la desertificación.
  evolucion:
    |
      Pertenece a la familia Meliaceae, un linaje de árboles tropicales conocido por la síntesis de limonoides, metabolitos secundarios de defensa que evolucionaron como mecanismo de protección frente a insectos herbívoros y hongos fitopatógenos. Esta adaptación bioquímica es la base de su moderno aprovechamiento como biopesticida natural.
comercio:
  exportacion:
    - pais: "Dinámica de Exportación"
      detalle: "India mantiene un monopolio casi absoluto en la exportación de extracto puro de neem, aceite crudo y torta de neem para fertilización orgánica. Países del sudeste asiático muestran un crecimiento emergente."
  importacion:
    - pais: "Dinámica de Importación"
      detalle: "Los mercados con regulaciones agrícolas verdes estrictas son los principales compradores, destacando la Unión Europea, Estados Unidos, Canadá y Brasil (para su sector agroindustrial extensivo)."
compuestosQuimicos:
  - nombre: "Limonoides (Azadiractina)"
    detalle: "Bloquea el sistema endocrino de los insectos y destruye paredes celulares de hongos fitopatógenos."
  - nombre: "Compuestos Azufrados"
    detalle: "Otorgan al aceite de la semilla propiedades bactericidas y antisépticas."
multimediaPrincipal:
  imagenUrl: "https://res.cloudinary.com/hi6t0jpv/image/upload/v1783717446/Neem_qfxhoa.webp"
  imagenPublicId: ""
  videoUrl: ""
  videoPublicId: ""
  proveedor: "PICSUM"
estado: "ACTIVO"
---

El Neem es un árbol perennifolio de valor extraordinario. Combina una potente acción medicinal humana con una revolucionaria aplicación agroecológica como biopesticida sistemático biodegradable.
