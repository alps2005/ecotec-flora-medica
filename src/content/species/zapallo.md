---
nombreComun: "Zapallo / Calabaza / Auyama"
nombreCientifico: "Cucurbita maxima"
nombresAlternativos: ["Calabaza", "Auyama"]
taxonomia:
  reino: "Plantae"
  division: "Magnoliophyta"
  clase: "Magnoliopsida"
  familia: "Cucurbitaceae"
  genero: "Cucurbita"
etnobotanica:
  clasificacion: "Alimenticia - Medicinal"
  parteUtilizada: "Semillas y fruto (pulpa)"
  usoTradicional: "Antiparasitaria, antiinflamatoria prostática y demulcente"
perfilEtnobotanico:
  |
    El uso principal de sus semillas es antiparasitario, paralizando los helmintos intestinales de manera segura. Su aceite regula y desinflama tejidos urológicos benignos.
historiaEvolucion:
  origen:
    |
      Nativa de América del Sur, domesticada en la región andina a partir de variantes silvestres milenarias.
  dispersion:
    |
      Difundida con rapidez mundial post-intercambio colombino, seleccionada en múltiples cultivares de gran resistencia de almacenamiento.
  evolucion:
    ""
comercio:
  exportacion:
    - pais: "Dinámica de Exportación"
      detalle: "México, España y Nueva Zelanda son los exportadores dominantes a nivel mundial de calabazas y zapallos, cubriendo ventanas estacionales clave en hemisferios opuestos."
  importacion:
    - pais: "Dinámica de Importación"
      detalle: "Los Estados Unidos representan el mercado de importación más grande. En Asia, Japón destaca como un comprador masivo de variedades dulces de Cucurbita maxima desde el Hemisferio Sur."
compuestosQuimicos:
  - nombre: "Cucurbitina"
    detalle: "Aminoácido pirrolidínico con probada acción antihelmíntica que paraliza los parásitos."
  - nombre: "Fitosteroles (Beta-sitosterol)"
    detalle: "Bloquea de forma competitiva hormonas asociadas a la hiperplasia prostática benigna."
multimediaPrincipal:
  imagenUrl: "https://antropocene.it/wp-content/uploads/2018/11/Cucurbita_maxima.jpg"
  imagenPublicId: ""
  videoUrl: ""
  videoPublicId: ""
  proveedor: "PICSUM"
estado: "ACTIVO"
---

El zapallo es uno de los cultivos domesticados más antiguos de América. Sus semillas albergan compuestos fitoquímicos específicos de alta eficacia clínica contra parasitosis y problemas prostáticos.
