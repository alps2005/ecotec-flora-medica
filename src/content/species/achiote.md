---
nombreComun: "Achiote"
nombreCientifico: "Bixa orellana"
nombresAlternativos: []
taxonomia:
  reino: "Plantae"
  division: "Magnoliophyta"
  clase: "Magnoliopsida"
  familia: "Bixaceae"
  genero: "Bixa"
etnobotanica:
  clasificacion: "Medicinal y tintórea"
  parteUtilizada: "Semillas"
  usoTradicional: "Antioxidante"
perfilEtnobotanico:
  |
    Uso medicinal y tintóreo. Las semillas y sus extractos son ricos en pigmentos antioxidantes que protegen contra el daño de los radicales libres. La medicina tradicional usa infusiones de sus hojas y semillas para inflamaciones de la próstata, problemas urinarios y como cicatrizante.
historiaEvolucion:
  origen:
    |
      Nativo de América Tropical (Amazonía y Centroamérica). Los pueblos precolombinos lo utilizaban como pintura corporal, repelente de insectos, especia y medicina.
  dispersion:
    |
      Los europeos lo llevaron a Asia (especialmente Filipinas) y África, donde se integró en la gastronomía local.
  evolucion:
    |
      Pertenece a la familia Bixaceae. Es un cultivo rústico adaptado a zonas cálidas y húmedas.
comercio:
  exportacion:
    - pais: "Perú, Brasil, Kenia y Costa de Marfil"
      detalle: "Principales exportadores de semilla de achiote y extractos de bixina."
  importacion:
    - pais: "Unión Europea, Estados Unidos y Japón"
      detalle: "Importan el colorante (Annatto / E-160b) como alternativa natural a los colorantes sintéticos en alimentos (quesos, mantequillas, snacks)."
compuestosQuimicos:
  - nombre: "Bixina y Norbixina"
    detalle: "Carotenoides responsables de la pigmentación roja-anaranjada y con alta actividad antioxidante."
  - nombre: "Tocotrienoles (Vitamina E)"
    detalle: "Potentes antioxidantes que protegen las membranas celulares."
  - nombre: "Flavonoides"
    detalle: "Presentes en las hojas, aportan acción antiinflamatoria."
multimediaPrincipal:
  imagenUrl: "https://upload.wikimedia.org/wikipedia/commons/a/a2/Achiote_pj.JPG"
  imagenPublicId: ""
  videoUrl: ""
  videoPublicId: ""
  proveedor: "PICSUM"
estado: "ACTIVO"
---

El achiote (Bixa orellana) es un arbusto originario de las regiones tropicales de América. Sus semillas están cubiertas por un arilo rojo que se utiliza mundialmente como colorante natural (bixina). Además de su uso industrial y culinario, posee compuestos bioactivos con gran potencial antioxidante y protector celular.
