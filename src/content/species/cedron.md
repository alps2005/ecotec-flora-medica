---
nombreComun: "Cedrón"
nombreCientifico: "Aloysia citrodora"
nombresAlternativos: ["Hierbaluisa"]
taxonomia:
  reino: "Plantae"
  division: "Magnoliophyta"
  clase: "Magnoliopsida"
  familia: "Verbenaceae"
  genero: "Aloysia"
etnobotanica:
  clasificacion: "Medicinal aromática"
  parteUtilizada: "Hojas"
  usoTradicional: "Digestiva"
perfilEtnobotanico:
  |
    Planta medicinal aromática. La infusión de sus hojas se consume tradicionalmente después de las comidas para facilitar la digestión, expulsar gases (carminativo) y aliviar dolores estomacales. También tiene propiedades relajantes que ayudan a combatir el estrés leve.
historiaEvolucion:
  origen:
    |
      Endémico de Sudamérica (principalmente de Chile, Argentina, Perú y Uruguay).
  dispersion:
    |
      Fue introducido en Europa a finales del siglo XVIII por botánicos españoles, popularizándose rápidamente en los jardines botánicos y como hierba aromática.
  evolucion:
    |
      Familia Verbenaceae. Adaptable a climas soleados y suelos bien drenados.
comercio:
  exportacion:
    - pais: "Marruecos, Paraguay y Chile"
      detalle: "Destacan en la producción para exportación de hojas secas y aceites."
  importacion:
    - pais: "Francia y Alemania"
      detalle: "Importan el aceite esencial para la industria de la perfumería fina y aromaterapia."
compuestosQuimicos:
  - nombre: "Citral (Geranial y Neral)"
    detalle: "Responsable de su característico aroma a limón y de sus efectos antimicrobianos."
  - nombre: "Limoneno y Linalol"
    detalle: "Aceites esenciales con propiedades sedantes y digestivas."
  - nombre: "Verbascósido"
    detalle: "Potente compuesto antioxidante."
multimediaPrincipal:
  imagenUrl: "https://picsum.photos/seed/cedron/1200/800"
  imagenPublicId: ""
  videoUrl: ""
  videoPublicId: ""
  proveedor: "PICSUM"
estado: "ACTIVO"
---

El cedrón (Aloysia citrodora), también conocido como hierbaluisa, es un arbusto caducifolio muy apreciado por el intenso aroma a limón de sus hojas. Es utilizado frecuentemente en la gastronomía, la perfumería y, de manera muy destacada, en la medicina tradicional sudamericana por sus propiedades digestivas y relajantes.
