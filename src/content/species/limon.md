---
nombreComun: "Limón / Limonero"
nombreCientifico: "Citrus limon"
nombresAlternativos: ["Limonero"]
taxonomia:
  reino: "Plantae"
  division: "Magnoliophyta"
  clase: "Magnoliopsida"
  familia: "Rutaceae"
  genero: "Citrus"
etnobotanica:
  clasificacion: "Alimenticia - Medicinal"
  parteUtilizada: "Fruto (jugo y cáscara / pericarpio)"
  usoTradicional: "Antiséptico, antioxidante, astringente y depurativo"
perfilEtnobotanico:
  |
    Su caracterización etnobotánica dual integra la pulpa ácida y la corteza. Tradicionalmente se emplea el jugo fresco como antimicrobiano y estimulante gástrico.
historiaEvolucion:
  origen:
    |
      Regiones nororientales de la India, expandiéndose tempranamente hacia Asia Central.
  dispersion:
    |
      Introducido en el Mediterráneo por los árabes durante la Edad Media y llevado a América en los viajes de Cristóbal Colón.
  evolucion:
    ""
comercio:
  exportacion:
    - pais: "Dinámica de Exportación"
      detalle: "España lidera la exportación mundial de limones frescos, seguida de cerca por México (dominante en el mercado norteamericano) y Turquía. Argentina destaca como el principal procesador de derivados industriales."
  importacion:
    - pais: "Dinámica de Importación"
      detalle: "Los Estados Unidos, los Países Bajos (como hub de distribución europeo), Alemania, Francia y Japón representan los mercados de importación más estables."
compuestosQuimicos:
  - nombre: "Ácidos Orgánicos Libres"
    detalle: "Alta concentración de ácido cítrico (5-7% en jugo), condicionando un pH bactericida."
  - nombre: "Compuestos Fenólicos"
    detalle: "Presencia de eriocitrina y hesperidina con propiedades vasoprotectoras."
multimediaPrincipal:
  imagenUrl: "https://res.cloudinary.com/hi6t0jpv/image/upload/v1783662651/Limon_pqfdqt.webp"
  imagenPublicId: ""
  videoUrl: ""
  videoPublicId: ""
  proveedor: "PICSUM"
estado: "ACTIVO"
---

El limón es un recurso fitoterapéutico global dotado de una versatilidad bioquímica excepcional. Su marcada acidez lo convierte en un potente agente antiséptico y antioxidante.
