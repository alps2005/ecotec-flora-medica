---
nombreComun: "Mandarina / Mandarino"
nombreCientifico: "Citrus reticulata"
nombresAlternativos: ["Mandarino"]
taxonomia:
  reino: "Plantae"
  division: "Magnoliophyta"
  clase: "Magnoliopsida"
  familia: "Rutaceae"
  genero: "Citrus"
etnobotanica:
  clasificacion: "Alimenticia - Medicinal"
  parteUtilizada: "Fruto (pulpa, jugo y endocarpio)"
  usoTradicional: "Vitamínica, digestiva y moduladora metabólica"
perfilEtnobotanico:
  |
    Valorada por su acción eúptica y procinética. El consumo de su pulpa se asocia directamente con el fortalecimiento inmunológico estacional, mientras que su corteza se emplea para mitigar la dispepsia.
historiaEvolucion:
  origen:
    |
      Nativa de las regiones tropicales y subtropicales del sureste de China y de Indochina, con registros de cultivo milenarios.
  dispersion:
    |
      Su introducción a Europa ocurrió a principios del siglo XIX a través de cargamentos comerciales ingleses, propagándose rápidamente por el Mediterráneo y América.
  evolucion:
    ""
comercio:
  exportacion:
    - pais: "Dinámica de Exportación"
      detalle: "China lidera el volumen global de exportación de mandarinas y tangerinas. En el Hemisferio Sur, Sudáfrica y Perú registran picos de exportación en contraestación hacia mercados norteamericanos y europeos."
  importacion:
    - pais: "Dinámica de Importación"
      detalle: "Estados Unidos, Rusia y la Unión Europea (liderada por Alemania y Francia) se consolidan como los principales importadores debido a la alta demanda de variedades fáciles de pelar (easy-peelers)."
compuestosQuimicos:
  - nombre: "Monoterpenos"
    detalle: "Aceite esencial del pericarpio rico en D-limoneno (65-75%) y gamma-terpineno."
  - nombre: "Flavonoides Polietoxilados (PMFs)"
    detalle: "Contiene altas concentraciones de nobiletina y tangeretina con propiedades antioxidantes y neuroprotectoras."
multimediaPrincipal:
  imagenUrl: "https://picsum.photos/seed/mandarina/1200/800"
  imagenPublicId: ""
  videoUrl: ""
  videoPublicId: ""
  proveedor: "PICSUM"
estado: "ACTIVO"
---

La mandarina es una especie ancestral pura del género Citrus. Su facilidad de pelado, perfil sensorial dulce y marcados atributos digestivos y vitamínicos la posicionan como un recurso etnobotánico y agroindustrial clave a nivel global.
